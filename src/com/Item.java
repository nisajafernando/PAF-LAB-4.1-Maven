...
public class Item {
	...
	public Connection connect() {
		...
		try {
			...
			con = DriverManager.getConnection("jdbc:mysql://127.0.0.1:1433/lab3", "root", "");
			// For testing
		...
		try {
			...
			while (rs.next()) {
				...
				output += "<td><form method='post' action='items.jsp'>" + "<input name='itemID' type='hidden' value='"
						+ itemID + "'>" + "<input name='action' value='select' type='hidden'>"
						+ "<input name='btnUpdate' type='submit' value='Update' class='btn btn-danger'>"
						+ "</form></td>" + "<td><form method='post' action='items.jsp'>"
